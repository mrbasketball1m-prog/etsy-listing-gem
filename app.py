...
#   DeepSeek (дёшево):  LLM_BASE_URL=https://api.deepseek.com           LLM_MODEL=deepseek-chat
#   Gemini Flash-Lite:  LLM_BASE_URL=https://generativelanguage.googleapis.com/v1beta/openai/
#                       LLM_MODEL=gemini-2.0-flash-lite
#   OpenRouter:         LLM_BASE_URL=https://openrouter.ai/api/v1       LLM_MODEL=<любая дешёвая>
client = OpenAI(
...
# В памяти, сбрасывается при редеплое — для MVP норм. Позже в Postgres.
ip_usage = defaultdict(lambda: {"day": date.today().isoformat(), "count": 0})
DAILY_IP_CAP = 30  # бэкстоп; основной триал-счётчик живёт на фронте (localStorage)
...
# --- Промпт = ТВОЙ МОАТ. Качество живёт здесь, а не в коде. Тюнишь его под продажи. ---
